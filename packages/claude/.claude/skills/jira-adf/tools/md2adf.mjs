#!/usr/bin/env node
// GENERATED by `npm run build` from md2adf.src.mjs - do not edit. See README.md.

// tools/md2adf.src.mjs
...
// node_modules/marklassian/dist/index.js
var generateLocalId = () => globalThis.crypto.randomUUID();
function parseAdfTag(raw) {
  const match = raw.trim().match(/^<adf>([\s\S]*?)<\/adf>$/i);
  if (!match)
    return null;
  const json = match[1].trim();
  if (json.length === 0) {
    throw new Error("<adf> tag content is empty \u2014 expected a JSON object or array");
  }
  let parsed;
  try {
  ...
  if (Array.isArray(parsed)) {
    return parsed.map((item, i) => {
      if (typeof item !== "object" || item === null || Array.isArray(item)) {
        throw new Error(`ADF node must be a JSON object or array of objects (item ${i} is not an object)`);
      }
      const node2 = item;
      if (typeof node2.type !== "string" || node2.type.length === 0) {
  ...
  if (typeof parsed !== "object" || parsed === null) {
    throw new Error(`ADF node must be a JSON object or array, got: ${JSON.stringify(parsed)}`);
  }
  const node = parsed;
  ...
  return tokens.map((token) => {
    switch (token.type) {
      ...
      case "list":
        ...
        if (allItemsAreTasks && token.items.some((item) => item.task)) {
          return {
            type: "taskList",
...
function processTable(token) {
  ...
  const rows = token.rows.map((row) => ({
    ...
    content: row.map((cell) => {
      const content2 = processParagraph(cell.tokens);
...
function processListItem(item) {
  ...
  (item.tokens || []).forEach((token) => {
    if (token.type === "text" || token.type === "em" || token.type === "strong" || token.type === "del" || token.type === "link" || token.type === "codespan") {
      currentParagraphTokens.push(token);
    } else {
      if (currentParagraphTokens.length) {
        itemContent.push({
      ...
      if (token.type === "list") {
        ...
        if (allItemsAreTasks && token.items.some((nestedItem) => nestedItem.task)) {
          itemContent.push({
            type: "taskList",
            attrs: { localId: generateLocalId() },
...
function getSafeText(token) {
  if (token.tokens?.length === 1 && token.tokens[0] && "text" in token.tokens[0]) {
...
function read(src) {
  try {
    return readFileSync(src === void 0 || src === "-" ? 0 : src, "utf8");
  } catch (err) {
    console.error(`md2adf: cannot read ${src === void 0 ? "stdin" : src}: ${err.message}`);
    process.exit(1);
  }
}
var md = read(arg);
